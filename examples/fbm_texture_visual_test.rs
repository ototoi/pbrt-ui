...
fn main() {
    ...
    let texture = Texture::new("fbm_default", "float", "fbm", None, &props, &Matrix4x4::default());
    let dependencies = HashMap::new();
    
    if let Some(DynaImage::ImageLuma32F(img)) = render_texture_image(&texture, &dependencies, TextureSizeType::Display) {
        // Convert to 8-bit for saving
        let img_u8: image::ImageBuffer<image::Luma<u8>, Vec<u8>> = image::ImageBuffer::from_fn(
            img.width(),
            img.height(),
            |x, y| {
        ...
        img_u8.save("/tmp/fbm_default.png").expect("Failed to save default FBM texture");
    ...
    let texture_high = Texture::new("fbm_high_octaves", "float", "fbm", None, &props_high, &Matrix4x4::default());
    
    if let Some(DynaImage::ImageLuma32F(img)) = render_texture_image(&texture_high, &dependencies, TextureSizeType::Display) {
        let img_u8: image::ImageBuffer<image::Luma<u8>, Vec<u8>> = image::ImageBuffer::from_fn(
            img.width(),
            img.height(),
            |x, y| {
        ...
        img_u8.save("/tmp/fbm_high_octaves.png").expect("Failed to save high octaves FBM texture");
    ...
    let texture_smooth = Texture::new("fbm_smooth", "float", "fbm", None, &props_smooth, &Matrix4x4::default());
    
    if let Some(DynaImage::ImageLuma32F(img)) = render_texture_image(&texture_smooth, &dependencies, TextureSizeType::Display) {
        let img_u8: image::ImageBuffer<image::Luma<u8>, Vec<u8>> = image::ImageBuffer::from_fn(
            img.width(),
            img.height(),
            |x, y| {
        ...
        img_u8.save("/tmp/fbm_smooth.png").expect("Failed to save smooth FBM texture");
    ...
    let texture_rough = Texture::new("fbm_rough", "float", "fbm", None, &props_rough, &Matrix4x4::default());
    
    if let Some(DynaImage::ImageLuma32F(img)) = render_texture_image(&texture_rough, &dependencies, TextureSizeType::Display) {
        let img_u8: image::ImageBuffer<image::Luma<u8>, Vec<u8>> = image::ImageBuffer::from_fn(
            img.width(),
            img.height(),
            |x, y| {
        ...
        img_u8.save("/tmp/fbm_rough.png").expect("Failed to save rough FBM texture");
