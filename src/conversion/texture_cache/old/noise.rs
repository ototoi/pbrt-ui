...
fn smooth_step(min: f32, max: f32, value: f32) -> f32 {
    let v = f32::clamp((value - min) / (max - min), 0.0, 1.0);
...
pub fn noise(x: f32, y: f32, z: f32) -> f32 {
    ...
    let ix = f32::floor(x) as i32;
    let iy = f32::floor(y) as i32;
    let iz = f32::floor(z) as i32;
    let dx = x - ix as f32;
    let dy = y - iy as f32;
    let dz = z - iz as f32;
...
pub fn fbm(p: &Vector3, dpdx: &Vector3, dpdy: &Vector3, omega: f32, max_octaves: u32) -> f32 {
    // Compute number of octaves for antialiased FBm
    let len2 = f32::max(dpdx.length_squared(), dpdy.length_squared());
    let n = f32::clamp(-1.0 - 0.5 * f32::log2(len2), 0.0, max_octaves as f32);
    let n_int = f32::floor(n) as usize;
