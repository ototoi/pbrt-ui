...
fn create_local_uniform_buffer(device: &wgpu::Device, num_items: usize) -> wgpu::Buffer {
    ...
    device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("ShadowMapRenderer Local Uniform Buffer"),
        size: required_size,
...
impl ShadowMapRenderer {
    pub fn new(device: &wgpu::Device) -> Self {
        ...
        let global_bind_group_layout =
            device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                label: Some("ShadowMapRenderer Global Bind Group Layout"),
        ...
        let local_bind_group_layout =
            device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                label: Some("ShadowMapRenderer Local Bind Group Layout"),
                entries: &[wgpu::BindGroupLayoutEntry {
                    binding: 0,
                    visibility: wgpu::ShaderStages::VERTEX | wgpu::ShaderStages::FRAGMENT,
                    ty: wgpu::BindingType::Buffer {
        ...
        let local_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("ShadowMapRenderer Local Bind Group"),
        ...
        let directional_shadow_info_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("ShadowMapRenderer Directional Shadow Info Buffer"),
    ...
    fn ensure_directional_shadow_pipeline(&mut self, device: &wgpu::Device) {
        ...
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            label: Some("ShadowMapRenderer Directional Shadow Pipeline"),
            ...
            for cascade in &shadow.cascades {
                ...
                let global_uniform_buffer =
                    device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
                        label: Some("ShadowMapRenderer Global Uniform Buffer"),
                        contents: bytemuck::bytes_of(&global_uniforms),
                        usage: wgpu::BufferUsages::UNIFORM | wgpu::BufferUsages::COPY_DST,
                    });
                let global_bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
                    label: Some("ShadowMapRenderer Global Bind Group"),
                ...
                let shadow_texture = &cascade.texture;
                {
                    let mut render_pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                        label: Some("ShadowMapRenderer Directional Shadow Render Pass"),
                        color_attachments: &[],
    ...
    fn create_directional_shadow_map_array(
        ...
        let texture = device.create_texture(&wgpu::TextureDescriptor {
            label: Some("ShadowMapRenderer Directional Shadow Map Array"),
            size: wgpu::Extent3d {
                width,
                height,
        ...
        let view = texture.create_view(&wgpu::TextureViewDescriptor {
            label: Some("ShadowMapRenderer Directional Shadow Map Array View"),
