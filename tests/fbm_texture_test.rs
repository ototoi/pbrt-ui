    ...
    if let Some(DynaImage::ImageLuma32F(img)) = result {
        ...
        for pixel in img.pixels() {
            ...
            assert!(
                ...
                "Pixel values should be in [0, 1] range, got {}",
                value
            );
        }
        ...
        assert!(
            has_variation,
            "FBM texture should have variation, not be constant"
        );
    } else {
...
fn test_fbm_texture_with_custom_params() {
    ...
    assert!(
        result.is_some(),
        "FBM texture with custom params should render successfully"
    );
...
fn test_fbm_texture_render_size() {
    ...
    assert!(
        result.is_some(),
        "FBM texture at render size should render successfully"
    );
